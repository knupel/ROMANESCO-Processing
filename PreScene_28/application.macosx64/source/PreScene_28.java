...
public class PreScene_28 extends PApplet {
...
public void opening() {
  ...
  if (!testRomanesco && openControleur) {
    ...
    stroke(blanc) ;
    textSize(48) ;
    text(version, sin(frameCount * .05f) *width, height/2 ) ;
  }
  if (!testRomanesco) { 
    if (openScene)      {
...
class Horloge extends Romanesco {
  ...
  public void display() {
    ...
    if(motion[IDobj]) {
      ...
      float speed_x = speedObj[IDobj] *.02f ;
      float speed_y = speedObj[IDobj] *.01f ;
      float speed_z = speedObj[IDobj] *.03f ;
...
class Boids extends Romanesco {
  ...
  public void display() {
    ...
    if(allBeats(IDobj) > beat_sensibility) {      
      ...
      float pos_x = sin(frameCount *.003f) *canvasXObj[IDobj] ;
      float pos_y = cos(frameCount *.003f) *canvasYObj[IDobj] ;
      flock.goal(pos_x,pos_y, depthGoal);
    }
    ...
    // SPEED
    float speed = map(speedObj[IDobj],0,1,.1f,7) ;
    speed *= speed ;
    if(sound[IDobj] )speed *= (map(mix[IDobj],0,1,.00000001f,7)) ;
    if(!motion[IDobj] || (sound[IDobj] && getTimeTrack() < .2f)) speed = .00000001f ;
    flock.speed(speed) ;
...
class SpiraleRomanesco extends Romanesco {
  ...
  public void display() {
    ...
    if(motion[IDobj]) {
      ...
      s *= s ;
      if(reverse[IDobj]) speed = s *tempo[IDobj] ; else speed = s *tempo[IDobj] *-1.f ;
    } else { 
      speed = 0.0f ;
    }
    //sound volume
    ...
    float maxValueCanvas = 3 *(kick[IDobj] *.7f) ;
...
class Kofosphere extends Romanesco {
  ...
  public void display() {
    ...
    float radius = map(canvasXObj[IDobj], width/10, width, .01f, 1.1f) ;
...
class Lignes extends Romanesco {
  ...
  public void display() {
    ...
    //quantity
    int num = (int)map(quantityObj[IDobj], 0, 1, canvas.x *.5f, canvas.y *.05f) ;
...
class Damier extends Romanesco {
  ...
  public void display() {
    ...
    //quantity
    int q = PApplet.parseInt(map(quantityObj[IDobj], 0, 1, 2, 9)) ;
    if(fullRendering) q *= q ;

    //amp
    float amp = map(amplitudeObj[IDobj],0,1, .3f, width *.007f) ;
    amp *= amp ;
...
class Webcam extends Romanesco {
  ...
  public void display() {
    ...
    float size_x = map(sizeXObj[IDobj],0.1f,width, minVal, maxVal) *snare[IDobj] ;
    float size_y = map(sizeYObj[IDobj],0.1f,width, minVal, maxVal) *kick[IDobj] ;
    float size_z = map(sizeZObj[IDobj],0.1f,width, minVal, maxVal) *hat[IDobj] ;
  ...
  public void displayPix(int mode, PVector hsb) {
    ...
    float factorSizeZ = map(sizeZObj[IDobj], .1f, width, .01f, height/100) ;
...
class Surface extends Romanesco {
  ...
  public void display() {
    ...
    if(mode[IDobj] != 0 ) {
      //size pixel triangle
      int sizePixMin = 7 ;
      int sizePix_grid_simple = PApplet.parseInt(sizePixMin +sizeXObj[IDobj] /11) ;
      ...
      // amplitude
      amplitude_simple_grid = amplitudeObj[IDobj] *height *.07f *allBeats(IDobj)  ;
...
// P3D SETUP
////////////
public void P3D_setup(int numObj, int numSettingCamera, int numSettingOrientationObject) {
...
public void light_position_draw(PVector mouse, int wheel) {
...
/////////
// Vec2
public Vec2 jitterVec2(int range) {
  Vec2 jitter = Vec2() ;
  jitter.x = random(-range, range) ;
  jitter.y = random(-range, range) ;
  return jitter ;
}
public Vec2 jitterVec2(int range_x, int range_y) {
  Vec2 jitter = Vec2() ;
...
// Vec3
public Vec3 jitterVec3(int range) {
  Vec3 jitter = Vec3() ;
  jitter.x = random(-range, range) ;
  jitter.y = random(-range, range) ;
...
public Vec3 jitterVec3(int range_x, int range_y, int range_z) {
  Vec3 jitter = Vec3() ;
  jitter.x = random(-range_x, range_x) ;
  jitter.y = random(-range_y, range_y) ;
...
// Vec4
public Vec4 jitterVec4(int range) {
  Vec4 jitter = Vec4() ;
  jitter.x = random(-range, range) ;
  jitter.y = random(-range, range) ;
  jitter.z = random(-range, range) ;
...
public Vec4 jitterVec4(int range_x, int range_y, int range_z, int range_w) {
  Vec4 jitter = Vec4() ;
  jitter.x = random(-range_x, range_x) ;
  jitter.y = random(-range_y, range_y) ;
  jitter.z = random(-range_z, range_z) ;
...
// VEC 2 from angle
///////////////////
public Vec2 fromRadians(float angle) {
  float x = (float)Math.cos(angle) ;
  float y = (float)Math.sin(angle) ;
  return Vec2(x,y) ;
}

public Vec2 fromDegrees(float angle) {
  angle = radians(angle) ;
  float x = (float)Math.cos(angle) ;
  float y = (float)Math.sin(angle) ;
  return Vec2(x,y) ;
...
///////////////
  static public void main(String[] passedArgs) {
    String[] appletArgs = new String[] { "PreScene_28" };
    if (passedArgs != null) {
