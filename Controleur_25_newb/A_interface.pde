...
void constructorSliderButton() {
  ...
  for ( int i = 11 ; i < numButtonTotalObjects ; i++) {
    // group one
...
void rolloverInfoVignette(PVector pos, PVector size, int IDname, int IDfamily) {
  if (mouseX > pos.x && mouseX < (size.x + pos.x ) && mouseY > pos.y - 10 && mouseY <  (size.y + pos.y) -20 ) {
    ...
    if (IDfamily == 1 ) {
      text(objectName[1][IDname -1], mouseX   +fontPos.x, mouseY +fontPos.y) ;
      text(objectAuthor[1][IDname -1], mouseX +fontPos.x, mouseY +fontPos.y -20) ;
    }
    if (IDfamily == 2 ) {
      text(objectName[2][IDname -1], mouseX   +fontPos.x, mouseY +fontPos.y) ;
      text(objectAuthor[2][IDname -1], mouseX +fontPos.x, mouseY +fontPos.y -20) ;
    }
    if (IDfamily == 3 ) {
      text(objectName[3][IDname -1], mouseX   +fontPos.x, mouseY +fontPos.y) ;
      text(objectAuthor[3][IDname -1], mouseX +fontPos.x, mouseY +fontPos.y -20) ;
    }
  }
}
...
void dropdownSetup() {
  ...
  //FONT dropdown
  ///////////////
  println("Dropdown",numDropdown) ;
  posDropdown[lastDropdown] = new PVector(posButtonFont.x, posButtonFont.y, 0.1)  ; // x y is pos anz z is marge between the dropdown and the header
  sizeDropdownFont = new PVector (75, 13, 15 ) ;
  PVector posTextDropdownTypo = new PVector( 3, 10 )  ;
  dropdown[lastDropdown] = new Dropdown(policeDropdownList,   posDropdown[lastDropdown] , sizeDropdownFont, posTextDropdownTypo, colorBG, colorBoxIn, colorBoxOut, colorBoxText, texteInterface, sizeTexteInterface) ;
...
void dropdownDraw() {
  ...
  newPosDropdown = new PVector ( posDropdown[numDropdown].x - margeAroundDropdown  , posDropdown[numDropdown].y ) ;
  // println( posModeDropdown[i].y) ;
  if ( !insideRect(newPosDropdown, totalSizeDropdown) ) dropdown[numDropdown].locked = false;
  //give the value for sending to Scène and save
  saveR [7] = byte(dropdown[numDropdown].getSelection() +1 ) ;
...
void dropdownMousepressed() {
  ...
  if (dropdown[numDropdown] != null) {
    if (insideRect(posDropdown[numDropdown], sizeDropdownFont) && !dropdown[numDropdown].locked  ) {
      dropdown[numDropdown].locked = true;
    } else if (dropdown[numDropdown].locked) {
      int line = dropdown[numDropdown].selectDropdownLine();
      if (line > -1 ) {
        dropdown[numDropdown].whichDropdownLine(line);
        //to close the dropdown
        dropdown[numDropdown].locked = false;        
      } 
    }
  }
