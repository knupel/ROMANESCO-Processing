class Object extends SuperRomanesco {
  public Object() {
    ...
    romanescoSlider = "Hue fill,Saturation fill,Brightness fill,Alpha fill,Canvas X,Canvas Y,Canvas Z" ;
    /*
  ...
  void setting() {
   ...
   println(canvasXObj[IDobj],canvasYObj[IDobj],canvasZObj[IDobj]) ;
   ...
   int numOfBoid = 100; //amount of boids to start the program with
   birthPlace = pos.copy() ;
   // width specific color 
   // flock = new Flock(birthPlace, numOfBoid,colorBoid);
   // width camaieu color
   flock = new Flock(birthPlace, numOfBoid, fillObj[IDobj], maxColorRef, rangeAroundYourColor);
 ...
 //DRAW
  ...
  flock.run(size, alignment, unity);
  ...
  float influenceArea =  abs(sin(frameCount *.001) *200) ;
  flock.influence(influenceArea);
  
  // SPEED
  float speed = 1 + abs(sin(frameCount *.001) *3) ;
  flock.speed(speed) ;
  ...
  if(mousePressed) { 
   // flock.rebirth((int)random(50,800)) ;
   flock.rebirth(birthPlace, (int)random(50,800), fillObj[IDobj],maxColorRef, rangeAroundYourColor) ;
  }
    // INFO
    objectInfo[IDobj] = ("") ;
  }
