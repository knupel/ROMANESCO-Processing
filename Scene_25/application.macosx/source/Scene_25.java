...
class ArbreRomanesco extends SuperRomanesco {
  ...
  public void display() {
    ...
    float amplitude = map(amplitudeObj[IDobj], 0,1, 0,height *.2f) *ampSon ;
    int n = (ng+nd) ;
    //quantity
...
class MesAmis extends SuperRomanesco {
  ...
  public void display() {
    ...
    float speed = map(speedObj[IDobj],0,100, .00001f, .05f);
...
class Escargot extends SuperRomanesco {
  ...
  public void display() {
    ...
    if(img[IDobj] == null)  {
      ...
      //println(listEscargot.size(), maxVoronoiPoints, maxEntryPoints, pixelAnalyzeSize, radiusAnalyze) ;
    } 
  }
