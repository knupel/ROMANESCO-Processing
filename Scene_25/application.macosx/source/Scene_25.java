...
public void updateVar() {
  ...
  for(int i = 0 ; i < numGroup ; i++) {
    ...
    int maxSource = 100 ;
    float minSize = .1f ;
    thicknessRaw[i] = mapStartSmooth(valueSlider[i+1][10], minSource, maxSource, minSize, (height*.33f), 2) ;
...
class LesVieux extends SuperRomanesco {
  ...
  public void courbe() {
    ...
    //vitesse / speed
    if (motion[IDobj]) speedCurve  = speedObj[IDobj]*20 *tempo[IDobj] ; else speedCurve = 0.0f ;
    //amplitude
    float ampB = map(amplitudeObj[IDobj],0,1, -50, width *.5f) ;
  ...
  // TRIANGLE DOUBLE
  public void doubleTriangleSetting() {
  ...
  public void doubleTriangleDisplay() {
    ...
    // speed
    if(sound[IDobj]) speedObj[IDobj] = speedObj[IDobj] *tempo[IDobj] *100.0f ;
    ...
    for (int i=0 ; i < listTri.size() ; i++ ) {
      ...
      tri.display(pos, canvas, side.x, side.y, speedObj[IDobj] *100.0f) ;
    }
  }
}
//end object one
...
class Bezier extends Rotation {  
  ...
  public void affichageBezier (PVector pos, int z, float b1, float b2, float b3, float b4, float b5, float b6, float b7, float b8) {
    /*
...
class Tri {
  ...
  public void display (PVector pos, PVector canvas, float coteA, float coteB, float speed) {
...
class Atome extends SuperRomanesco {
  ...
  public void display() {
    ...
    for (Atom atm : atomList) {
      ...
      //diameter
      float fp = map (sizeXObj[IDobj], .1f,width, 1, width *.1f) ;
      ...
      //width
      float posTextInfo = map(sizeYObj[IDobj], .1f, width,fp*.2f, width*.2f) + (beat[IDobj] *2.0f)  ;
      //Canvas
  ...
  // ADD function
  public void atomAdd(PVector newPos) {
    //data
    //amplitude
    ...
    // Atom motion
    float startVel = 1.0f ;
    PVector posA = new PVector ( mouse[IDobj].x -newPos.x, mouse[IDobj].y -newPos.y, 0.0f ) ;
...
class Spray extends SuperRomanesco {
  ...
  public void addEncre(float fp, PVector d, float a, int spray, float dif, int flux) {
...
class SpiraleRomanesco extends SuperRomanesco {
  ...
  public void display() {
    ...
    n = nMax ;
    //amplitude
    float ratioScreen = height*.1f ;
    if (ratioScreen < 1 ) ratioScreen = 1 ;
    float ratio = 1.25f + (ratioScreen *.0005f) ; 
    float z = map(amplitudeObj[IDobj], 0,1,1.01f, ratio)  ;
    //speed
    ...
    float heightObj = width    *sizeYObj[IDobj] *volumeG *(.01f /width) *kick[IDobj] ;
    float widthObj = height   *sizeXObj[IDobj]  *volumeD *(.01f /height) *hat[IDobj] ;
    float depthObj = height   *sizeZObj[IDobj]  *volumeD *(.01f /width) *hat[IDobj] ;
...
class Spirale extends Rotation {  
  ...
  public void affichage (int n, int nMax, PVector size, float z, int mode) {
    n = n-1 ;
...
class ArbreRomanesco extends SuperRomanesco {
  ...
  public void display() {
    ...
    //size
    float x = sizeXObj[IDobj] *(g+d) *.1f ;
    float y = sizeYObj[IDobj] *(g+d) *.1f ;
    float z = sizeZObj[IDobj] *(g+d) *.1f ;
    ...
    //amplitude
    float amplitude = map(amplitudeObj[IDobj], 0,1, 0,height *.5f) *allBeats(IDobj) ;
...
//::::::::::::::::::::::::::::  
  public void actualisation (PVector posArbre, float e, PVector size, float propA, float propB, int fourcheA, int fourcheB, float amplitude, int n, int mode, float angle, float speed, int ID) {
    rotation += speed ;
  ...
  //float fourche = 10.0 ; 
  public void branch(float e, PVector proportion, float propA, float propB, int fourcheA, int fourcheB, float amplitude, int n, int mode, int ID) {
    PVector newSize = proportion.get() ;
    newSize.x = proportion.x /(random(propA, propB)*.3f) ;
    newSize.y = proportion.y /(random(propA, propB)*.3f) ;
    newSize.z = proportion.z /(random(propA, propB)*.3f) ;
...
class Soleil extends SuperRomanesco {
  ...
  public void display() {
    ...
    float jitting = cos(jitter) *tempo[IDobj] ;
     //noise
     float noise = 0 ;
     if (sound[IDobj]) noise = (mix[IDobj] *mix[IDobj]) *50.0f ; else noise = sq(amplitudeObj[IDobj] *10.0f) ;
    // rotation speed
    ...
    objectInfo[IDobj] = ("Motion "+revolution + " Radius revolution " + abs(PApplet.parseInt(mouse[IDobj].x -width/2)) ) ;
  ...
  // ANNEXE
  // soleil with jitter
  public void soleil(PVector pos, int diam, int numBeam, float jitter, float noise) {
...
class BaliseRomanesco extends SuperRomanesco {
  ...
  //DRAW
  public void display() {
    if (motion[IDobj]) speed = (map(speedObj[IDobj], 0,1, 0,20)) *tempo[IDobj] ; else speed = 0.0f ;
    // color and thickness
    aspect(IDobj) ;
    ...
    //factor size
    float factor = map(forceObj[IDobj],0,1,1,100) *(allBeats(IDobj) *.2f) ;
    ...
    balise.display(amp, var, sizeBalise, factor, PApplet.parseInt(radiusBalise), sound[IDobj], mode[IDobj]) ;
...
class Balise extends Rotation {  
  ...
  public void display (float amp, PVector var, PVector sizeBalise, float factor, int max, boolean sound, int mode) {
    pushMatrix() ;
    rectMode(CENTER) ;
...
class MesAmis extends SuperRomanesco {
  ...
  public void lineFriends(Ami ami) {
...
class Lignes extends SuperRomanesco {
  ...
  public void display() {
    ...
    //quantity
    float q = map(quantityObj[IDobj], 1, 100, canvas.x *.5f , canvas.y *.05f) ;
    //rotation
...
class Damier extends SuperRomanesco {
  ...
  public void display() {
    ...
    size.x = ( map(sizeXObj[IDobj],.1f,width,5,width/8) *penPressure *allBeats(IDobj) ) *g ;
    ...
    //quantity
    int q = PApplet.parseInt(map(quantityObj[IDobj], 1, 100, 2,15)) ;

    //amp
    float amp = map(amplitudeObj[IDobj],0,1, .1f, height/100) ;
...
class Rideau extends SuperRomanesco {
  ...
  public void display() {
    //END OF DON'T TOUCH
    //speed / vitesse
    float vitesse = 0.01f + (speedObj[IDobj] *0.1f) ;
    //dur\u00e9e / life
    int vie = PApplet.parseInt(map(lifeObj[IDobj],0,1,100,20000)) ;
    //thickness / \u00e9paisseur
...
class Curtain { 
  ...
  public void actualisation(float amplitude) {
  ...
  //SHAPE
  public void drawRect (PVector size, int ID) {
    //life
    vp = vp + chrono ;
    chrono = -1 ;
  ...
  public void drawBox (PVector size, int ID) {
    //life
    vp = vp + chrono ;
    chrono = -1 ; 
...
class Galaxie extends SuperRomanesco {
  ...
  public void display() {
    ...
    PVector surface = new PVector(marge.x *2.0f + width, marge.y *2.0f +height) ;
    
    if (mode[IDobj] == 0 ) numFromControler = PApplet.parseInt(3*(sq(quantityObj[IDobj])) ) ; else numFromControler = 30 + PApplet.parseInt(10 *quantityObj[IDobj]) ;
    ...
    if(motion[IDobj]) speedDust = map(speedObj[IDobj],0,1, 0.0001f ,3) ; else speedDust = 0.000001f ;
...
class Escargot extends SuperRomanesco {
  ...
  public void display() {
    ...
    if(img[IDobj] == null)  {
      ...
      objMotion = PApplet.parseInt(map(forceObj[IDobj],0,1, 0,20) *(1.0f + pen[IDobj].z)) ;
      ...
      int n = PApplet.parseInt(map(quantityObj[IDobj],0,100,10,150)) ;
      maxEntryPoints = n *n ;
      ...
      if(mode[IDobj] != 8 || (maxEntryPoints != maxEntryPointsRef && scene) ) {
       ...
       //size and thickness
       PVector sizePix = new PVector (map(sizeXObj[IDobj],.1f,width, 1, height/50 ), map(sizeYObj[IDobj],.1f,width, 1, height/50 ), map(sizeZObj[IDobj],.1f,width, 1, height/50 )) ;
      ...
      if (mode[IDobj] == 0 || mode[IDobj] == 255 ) {
        displayRawPixel(sizePix.x, opacityShapeIn, rangeReactivitySoundHundred, rangeReactivitySoundThreeHundredSixty, musicFactor, ratioImgWindow) ;
      } else if (mode[IDobj] == 1 ) {
        escargotRaw(sizePix.x, opacityShapeIn, rangeReactivitySoundHundred, rangeReactivitySoundThreeHundredSixty, musicFactor, ratioImgWindow) ;
      } else if (mode[IDobj] == 2 ) {
        escargotPoint(sizePix.x, opacityShapeIn, rangeReactivitySoundHundred, rangeReactivitySoundThreeHundredSixty, musicFactor, ratioImgWindow) ;
  ...
  public void displayRawPixel(float sizeP, int opacity, PVector rangeHundred, PVector rangeThreeHundredSixty, PVector musicFactor, PVector ratio) {
    ...
    for ( Pixel p : listPixelRaw ) {
      ...
      if(sound[IDobj]) strokeWeight(beatReactivityHSB(sizePixCtrl, p.size, p.colour, rangeThreeHundredSixty, rangeHundred, musicFactor ).z) ; else strokeWeight(sizeP) ;
  ...
  public void escargotPoint(float sizeP, int opacity, PVector rangeHundred, PVector rangeThreeHundredSixty, PVector musicFactor, PVector ratio) {
    ...
    for ( Pixel p : listEscargot ) {
      ...
      stroke(c, opacity) ;
      float strokeSound = beatReactivityHSB(sizePixCtrl, p.size, p.colour, rangeThreeHundredSixty, rangeHundred, musicFactor).z ;
      if(soundButton[IDobj] == 1 ) strokeWeight(strokeSound) ; else strokeWeight(p.size.x *sizeP) ;
      point(p.pos.x *ratio.x, p.pos.y *ratio.y) ; 
    }
  }
...
class Honeycomb extends SuperRomanesco {
  ...
  public void display() {
    ...
    if(hexagonStroke < 1.0f ) hexagonStroke = 1.0f ;
    
    //change the radius of hexagon
    hexagonRadius = map(sizeXObj[IDobj],.1f,width, 5, width/15)  ;
    ...
    // new honeycomb
    if((action[IDobj] && nTouch) || allBeats(IDobj) >= 3.125f ) newHoneycomb = true ;
...
class Hexagon {
  ...
  public void display(PVector[] v, int in) {
    pushMatrix();
    translate(x, y);
...
class Letter extends SuperRomanesco {
  ...
  public void display() {
    ...
    //speed
    float speed ;
    if(motion[IDobj]) speed = map(speedObj[IDobj], 0,1, 0.001f, 0.5f ) *tempo[IDobj]  ; else speed = 0.0f ;
    //to stop the move
  ...
  // ANNEXE
    ...
    public void letters(float speed, int axeLetter, int c, float thickness, PVector jttr) {
  ...
  public void wheelLetter(int num, float speed, int c, float thickness, PVector jttr) {
  ...
  public void displayLetter(int which, int c, float thickness, PVector ampJttr) {
  ...
  public PVector jitterPVector(PVector range) {
    float factor = 0.0f ;
    if(sound[IDobj]) factor = 2.0f ; else factor = 0.0f  ;
...
class Karaoke extends SuperRomanesco {
  public Karaoke() {
    ...
    romanescoRender = "classic" ;
    romanescoMode = "" ; // separate the name by a slash and write the next mode immadialtly after this one.
    romanescoSlider = "Hue fill,Saturation fill,Brightness fill,Alpha fill,Width,Canvas X,Canvas Y,Direction" ;
  }
  //GLOBAL
...
int artificialTime ;
//FOND
public void backgroundRomanesco(boolean useShader) {
  if(eBackground == 1) {
    int bg ;
...
public void displayInfoScene() {
  ...
  posInfo += 1 ;
  text("right " +PApplet.parseInt(input.right.get(1) *100),15, 15 *(posInfo)) ;  text("left " + PApplet.parseInt(input.left.get(1) *100), 50, 15 *(posInfo)) ;
  posInfo += 1 ;
}
...
public void repereCamera(PVector size) {
  if(modeP3D && displayInfo3D)  {
    PVector newSize =  PVector.mult(size,.1f) ;
    int xColor = rouge ;
    int yColor = vert ;
    int zColor = jaune ;
    int posTxt = 10 ;
...
public PVector follow(PVector finish, float speed) {
...
class Pixel {
  ...
  //CHANGE COLOR 
  // hue from Range
  public void changeHue(PVector HSBinfo, int[] newHue,  int[] start, int[] end) {
  ...
  public void changeSat(PVector HSBinfo, int[] newSat,  int[] start, int[] end) {
  ...
  // saturation from Range
  public void changeBright(PVector HSBinfo, int[] newBright,  int[] start, int[] end) {
...
public void fullscreenWithMavericks(int whichOne, boolean decor) {
  initNumberScreen() ;
  infoScreen() ;
  screenMode(decor, screenInfo[whichOne]) ;
}

public void sketchPosition(int whichOne) {
...
public void screenMode(boolean varFullscreen, PVector size) {
  if(varFullscreen) {
    removeBorder(true) ;
...
public void sketchPosWithoutMavericks(int x, int y, int whichOne) {
...
public PVector fullScreen(int whichOne) {
...
// width
public int screenWidth(int whichOne) {
  int x = 0 ;
  if (whichOne >= screenDevice.length ||  whichOne < 0 ) { 
    whichOne = 0 ;
    println( "screen choice not available") ;
...
//heigth
public int screenHeight(int whichOne) {
  int y = 0 ;
  if (whichOne >= screenDevice.length || whichOne < 0 ) { 
    whichOne = 0 ;
    println( "screen choice not available") ;
...
public void removeBorder(boolean decor) {
  frame.removeNotify();
...
//ANNEXE LIGHT VOID
public void romanescoLight(PVector colorOne, PVector colorTwo, PVector dirOne, PVector dirTwo) {
