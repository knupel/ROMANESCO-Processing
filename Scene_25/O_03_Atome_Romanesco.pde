...
//DRAW
void romanescoThreeDraw(String [] dataControleur, String [] dataMinim) {
...
int getFamillyRomanescoThree() { return romanescoThree.getIDfamilly() ; }
...
class RomanescoThree extends SuperRomanesco 
{
  ...
  void setting() {
   ...
   int Z = int(random (1, map (valueObj[IDobj][27], 0,100, 1,118))) ;
    ...
    for (Atom atm : atomList) {
      ...
      if(motion[IDobj]) {
        if(spaceTouch && actionButton[IDobj] == 1) {
          ...
          if (meteoButton[IDobj] == 1) newDirection = normalDir((int)motionMeteo[IDobj].x) ; else newDirection = normalDir(int(map(valueObj[IDobj][18],0,100,0,360))) ;
        }
      } else {
