  ...
  for(int i = 0 ; i < NUM_ITEM ; i++) {
    ...
    for(int j = 0 ; j <= NUM_ITEM ; j++) {
      if(ID == j ) { 
        ...
        // item_rank[j] =lookFor.getInt("Library Order") ;
...
Button_plus[] button_item, button_item_list ;
...
void init_button() {
  ...
  button_item_list = new Button_plus[NUM_ITEM +1] ;
  ...
  */
  ...
  for( int i = 0 ; i < button_item_list.length ; i++) {
    ...
    if(temp_item_info_split[0] != "" ) {
      if(i <= max_by_col ) {
        ...
        button_item_list[i] = new Button_plus(pos, size) ;
  ...
  for( int i = 0 ; i < button_item_list.length ; i++) {
    if(item_info[i] != "" ) {
      ...
      button_item_list[i].set_on_off(on_off_item_menu[i]) ;
  ...
  */
  if( NUM_ITEM > 0) {
    for(int i = 1 ; i < button_item_list.length ; i++) {
      ...
      if(button_item_list[i].get_on_off()) {
       //  println(i, rank) ;
        on_off_item_menu[ID] = true ; // use ID item
      } else { 
        on_off_item_menu[ID] = false ;
      }
    }
  }
