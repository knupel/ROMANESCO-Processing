...
public class PreScene_25 extends PApplet {
...
public void updateVar() {
  ...
  for(int i = 0 ; i < numGroup ; i++) {
    ...
    // future slider
    fontSizeRaw[i] = map(sizeXRaw[i], minSize, width, .01f, height *.05f) ;
...
class LesVieux extends SuperRomanesco {
  ...
  public void courbe() {
    ...
    //vitesse / speed
    if (motion[IDobj]) speedCurve  = speedObj[IDobj]*.2f *tempo[IDobj] ; else speedCurve = 0.0f ;
    //amplitude
...
class Soleil extends SuperRomanesco {
  ...
  public void display() {
    ...
    speedRotation = sq(speedObj[IDobj] *.1f *tempo[IDobj]) ;
...
class BaliseRomanesco extends SuperRomanesco {
  ...
  public void display() {
    ...
    //factor size
    float factor = forceObj[IDobj] *(allBeats(IDobj) *.2f) ;
...
class MesAmis extends SuperRomanesco {
  ...
  public void display() {
    ...
    float speed = map(speedObj[IDobj],0,100, .0001f, .2f);
    speed = speed*speed ;
...
class Lignes extends SuperRomanesco {
  ...
  public void display() {
    ...
    //speed
    if(motion[IDobj]) speed = map(speedObj[IDobj], 0,100, 0, height/20 ) * tempo[IDobj]  ; else speed = 0.0f ;
    ...
    //rotation
    rotation(directionObj[IDobj], canvas.x *.5f, canvas.y *.5f ) ;
    //display
...
class Damier extends SuperRomanesco {
  ...
  public void display() {
    ...
    //amp
    float amp = map(amplitudeObj[IDobj],0,1, .5f, 3) ;
  ...
  public void display() {
    ...
    if(img[IDobj] == null)  {
      ...
      if(mode[IDobj] != 8 || (maxEntryPoints != maxEntryPointsRef && scene) ) {
       ...
       //size and thickness
       PVector sizePix = new PVector (map(sizeXObj[IDobj],.1f,width, .1f, height/10 ), map(sizeYObj[IDobj],.1f,width, .1f, height/10 ), map(sizeZObj[IDobj],.1f,width, .1f, height/10 )) ;
...
class Letter extends SuperRomanesco {
  ...
  public void display() {
    ...
    //speed
    float speed ;
    if(motion[IDobj]) speed = map(speedObj[IDobj], 0,100, 0.001f, 0.5f ) *tempo[IDobj]  ; else speed = 0.0f ;
    //to stop the move
...
public void backgroundRomanescoPrescene(boolean useShader) {
  if(eBackground == 1) {
    int bg ;
...
public void backgroundShaderDraw(boolean renderP3D, boolean useShaderOrNot, int whichOne) {
  if(renderP3D && useShaderOrNot) {
    ...
    text("Shader is active", sizeText, sizeText*1.5f) ;
  } 
...
public void cursorDraw() {
  ...
  if(!mousepressed[0]) {
    if(pmouse[0].x != mouse[0].x || pmouse[0].y != mouse[0].y ) {
     pmouse[0] = gotoTarget(pmouse[0],  mouse[0], .1f) ;
    }
  } 
...
public PVector averagePosition(boolean leapMotionConnected) {
  if( numFingers() > 1 ) {
...
// ANNEXE
public PVector averageFingersPosition(PointableList objectNum, InteractionBox iBox) {
  ...
  if(averagePosOfTheFinger.x <= 0 ) averagePosOfTheFinger.x = 0.0001f ; 
  if(averagePosOfTheFinger.y <= 0 ) averagePosOfTheFinger.y = 0.0001f ; 
...
public PVector addFingerPosition(PointableList objectNum, InteractionBox iBox) {
...
public PVector averagePosition(boolean leapMotionConnected, PVector vertRange) {
  if( numFingers() > 1 ) {
    InteractionBox iBox = leap.frame().interactionBox();
    PointableList objectNum = leap.frame().pointables();
    if(leapMotionConnected) {
...
// ANNEXE
public PVector averageFingersPosition(PointableList objectNum, InteractionBox iBox, PVector vertRange) {
  PVector averagePosOfTheFinger = new PVector (0,0,0) ;
...
public PVector addFingerPosition(PointableList objectNum, InteractionBox iBox, PVector vertRange) {
    //quantity of finger in the area
  PVector addPos = new PVector() ;
  for(int p = 0; p < objectNum.count(); p++) {
    //initialization
...
// ANNEXE
public float changeTheRangeVert(PVector range , float axe) {
...
*/
  static public void main(String[] passedArgs) {
    String[] appletArgs = new String[] { "PreScene_25" };
    if (passedArgs != null) {
