...
public void updateVar() {
  ...
  for(int i = 0 ; i < numGroup ; i++) {
    ...
    // future slider
    fontSizeRaw[i] = map(sizeXRaw[i], 0, 100, .01f, height *.01f) ;
...
class BaliseRomanesco extends SuperRomanesco {
  ...
  public void display() {
    ...
    if (mode[IDobj] == 0 || mode[IDobj] == 255 ) {
      balise.baliseDisc (amp, var, sizeBalise, PApplet.parseInt(radiusBalise), sound[IDobj]) ;
    } else if (mode[IDobj] == 1 ) {
     balise.baliseRect (amp, var, sizeBalise, PApplet.parseInt(radiusBalise), sound[IDobj]) ;
    } 
...
class Balise extends Rotation {  
  ...
  public void baliseDisc (float amp, PVector var, PVector sizeBalise, int max, boolean sound ) {
    pushMatrix() ;
    rectMode(CENTER) ;
  ...
  public void baliseRect (float amp, PVector var, PVector sizeBalise, int max, boolean sound ) {
    pushMatrix() ;
    rectMode(CENTER) ;
    PVector inputResult = new PVector(0,0,0) ;
    if ( max > 512 ) max = 512 ;
...
class Letter extends SuperRomanesco {
  ...
  public void display() {
    ...
    //jitter
    int jitter = PApplet.parseInt(map(amplitudeObj[IDobj],0,1,1,15)) ;
...
public void displayInfoScene() {
  ...
  //INFO MOUSE and PEN
  text("position X " + mouseX + "  position Y " + mouseY + "  molette " + wheel[0] + "    pen orientation " + (int)deg360(pen[0]) +"\u00b0   stylet pressur " + PApplet.parseInt(pen[0].z *10),15, 15 * (posInfoObj) ) ;  
  posInfoObj += 1 ;
  //INFO SOUND
  ...
  posInfoObj += 1 ;
  text("right" +PApplet.parseInt(input.right.get(1) *100) + "  left " + PApplet.parseInt(input.left.get(1) *100) , 15,15 *(posInfoObj)) ; 
