...
class Simple extends Button {
 ...
 void buttonPicSerie(PImage[] vignette_OFF_in, PImage[] vignette_OFF_out, PImage[] vignette_ON_in, PImage[] vignette_ON_out, int wichVignette) {
   int correctionX = -1 ;
   if (onOff ) {
     if (rollover() && !dropdownActivity) image(vignette_ON_in[wichVignette],pos.x +correctionX, pos.y) ; else image(vignette_ON_out[wichVignette],pos.x +correctionX, pos.y) ;
   } else {
     if (rollover() && !dropdownActivity) image(vignette_OFF_in[wichVignette],pos.x +correctionX, pos.y) ; else image(vignette_OFF_out[wichVignette],pos.x +correctionX, pos.y) ;
   }
 }
